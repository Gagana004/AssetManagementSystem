...
$query .= "	
			SELECT 	d.dis_id, d.dis_date, d.dis_status,i.it_name, u.fname, a.ast_code,di.disposal_desc,
					(	SELECT 	ar.ar_code 
						FROM 	area AS ar 
						WHERE 	ar.ar_id = d.frm_area) AS `from`
			FROM 	dispatch as d, disposal as di, item as i, user as u, asset as a
			WHERE 	d.dis_id = di.d_dis_id AND
					d.it_id = i.it_id AND
        			d.u_id = u.u_id AND
...
if(isset($_POST["search"]["value"]))
{
	...
	$query .= 'OR di.disposal_desc LIKE "%'.$_POST["search"]["value"].'%" ';
