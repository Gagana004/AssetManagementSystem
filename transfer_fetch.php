...
$query .= "	SELECT 	d.dis_id, d.dis_date, d.dis_status,i.it_name, u.fname, a.ast_code,
					(	SELECT 	ar.ar_code 
						FROM 	area AS ar 
						WHERE 	ar.ar_id = d.frm_area) AS `from`,
					(	SELECT 	ar.ar_code 
						FROM 	area AS ar 
						WHERE 	ar.ar_id = t.tr_to_area) AS `to`
			FROM 	dispatch AS d, transfer AS t, item AS i, user AS u, asset AS a
			WHERE 	d.dis_id = t.tr_dis_id AND
					d.it_id = i.it_id AND
        			d.u_id = u.u_id AND
...
if(isset($_POST["search"]["value"]))
{
	...
	$query .= 'OR (	SELECT 	ar.ar_code 
					FROM 	area AS ar 	
					WHERE 	ar.ar_id = t.tr_to_area) LIKE "%'.$_POST["search"]["value"].'%" ';
